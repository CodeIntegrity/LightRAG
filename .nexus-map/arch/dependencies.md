...
- 可以确信：
  ...
  - WebUI 通过 HTTP 依赖 API 暴露的文档、图谱、检索、状态以及 `/prompt-config/*` 与 workspace 相关接口。
...
- inferred from file tree/manual inspection：
  ...
  - `PromptManagement` 页面、retrieval 临时版本选择以及 workspace 管理组合关系主要来自 `App.tsx`、`SiteHeader.tsx`、`RetrievalTesting.tsx`、`PromptManagement.tsx` 与相关测试文件的直接阅读。

## 修改建议

- 改 `lightrag/api/lightrag_server.py` 前，先跑 `query_graph.py --impact lightrag/api/lightrag_server.py`，因为它同时连接路由、模型绑定、共享存储、workspace 入口、健康摘要和 prompt 版本接口。
