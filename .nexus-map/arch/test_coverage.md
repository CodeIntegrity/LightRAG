...
这一组继续覆盖 chunking、实体抽取、工作区隔离和状态保留，也覆盖 prompt 默认模板、seed 版本、registry 原子写入、active group 解析，以及 query-time override 的生效顺序。

### 2. API、鉴权、Workspace 与 Prompt Config 接口
...
这组测试说明 API 层现在不仅覆盖原有 REST / 鉴权 / Ollama 兼容行为，也覆盖 `/prompt-config/*`、workspace 路由和 document rebuild / runtime 集成行为。
...
- `tests/test_postgres_performance_timing.py`
...
### 5. WebUI API Client、组件、Store 与工具函数
...
- `lightrag_webui/src/features/LoginPage.test.tsx`
...
当前前端测试面已扩展到 API client、store、workspace、graph workbench、login 页面与 prompt 管理，但仍以 Vitest 组件 / store / util 级为主。
...
- 改 `lightrag/api/routers/prompt_config_routes.py`、workspace 路由或 `/health`：
  - 先看 `tests/test_prompt_config_routes.py`
  - 再看 `tests/test_query_prompt_overrides_api.py`、`tests/test_workspace_runtime_app_integration.py`
- 改 `lightrag/kg/nebula_impl.py`：
  - 必看 `tests/test_nebula_graph_storage.py`
- 改 PostgreSQL / OpenSearch 存储：
  - 同步检查对应性能、分页、迁移和 upsert 测试簇
- 改 `scripts/setup/setup.sh`：
  - 必看 `tests/test_interactive_setup_outputs.py`
  - 同时检查 `tests/test_runtime_target_validation.py` 与 `tests/test_env_examples_completeness.py`
- 改 WebUI prompt 管理、workspace 管理或 graph workbench：
  - 至少看对应 `*.test.tsx` / `*.test.ts` 文件，以及 `lightrag.test.ts` / `lightrag.workspace.test.ts`
