...
- Prefer upstream-merge-friendly changes: keep diffs narrowly scoped, avoid drive-by reformatting/renames/moves, reuse existing abstractions and file boundaries where possible, and split mechanical cleanup or structural refactors into separate reviewable commits/PRs. When a change must diverge from upstream structure, document why, the compatibility impact, and what old path/behavior should be retired.
