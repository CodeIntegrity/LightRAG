...
* **Nebula 连接必需环境变量**：`NEBULA_HOSTS`、`NEBULA_USER`、`NEBULA_PASSWORD`。
  `NEBULA_PASSWORD` 必须显式设置；如果您的 NebulaGraph 部署允许空密码，它可以配置为空字符串。
...
* **图谱可视化**：WebUI 使用 `label=*` 加载图谱时可能触发较宽的图读取。NebulaGraphStorage 会复用 bounded BFS 已取得的 adjacency，并发加载独立的节点和边数据，并且只在内存中缓存排序后的 popular-label 结果。
* **Popular-label 缓存**：缓存仅作用于单个 `NebulaGraphStorage` 实例；miss 时从 NebulaGraph 重建；图写入、破坏性操作和索引重建回调会使其失效。`get_all_labels()` 仍然直接读取 NebulaGraph。
* **Custom KG 导入**：NebulaGraphStorage 通过现有 `upsert_nodes_batch()` 和 `upsert_edges_batch()` 图存储接口执行 grouped 批量节点/边写入，使 custom KG 导入和图构建避免逐行串行写入 fallback。
* **实验性读取路径**：VID-native `FETCH` 和 `GO` 遍历默认不启用。生产采用前应验证长 Unicode ID、转义、缺失节点行为、方向处理、深度语义、NebulaGraph 版本、数据规模、查询次数以及 p50/p95 延迟。
* **Listener 自动配置**：如果希望新建 workspace 自动注册 Nebula listener，优先显式设置 `NEBULA_LISTENER_HOSTS`（例如 `172.28.0.10:9789`）。
* **Listener 回退发现**：为避免隐藏的启动扫描，跨 space 的 listener 自动发现默认关闭；只有在您明确设置 `NEBULA_LISTENER_DISCOVERY=true` 时，LightRAG 才会尝试复用其他 Nebula space 中已发现的 listener 地址。
* **TLS 开关**：设置 `NEBULA_SSL=true` 后，LightRAG 会把 Nebula 的 SSL client config 传给 `nebula3-python`。
...
示例脚本：`examples/unofficial-sample/lightrag_openai_nebula_demo.py`。
