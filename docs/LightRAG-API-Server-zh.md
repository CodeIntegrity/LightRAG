...
当调用方已经确定好 chunk 边界，并希望 LightRAG 严格按照这些边界建图时，使用这个接口。

契约如下：

- 请求体包含 `full_text`、`text_chunks`，以及可选的 `doc_id`、`file_path`
- 服务端会写入 `doc_status` 记录，并设置 `metadata.source = "custom_chunks"`
- 如果提供了 `file_path`，服务端会把相同路径写入 `doc_status`、`full_docs` 和持久化后的 chunk 元数据
- 在开始实体/关系抽取前，先持久化 chunks
- 抽取与图谱合并在同一个请求内同步完成
- 该接口不会创建后台 `track_id`

关键行为：

- 该路径会保留调用方传入的 chunk 边界
- 它不同于 `/documents/text`、`/documents/texts`、`/documents/upload` 这些标准入口；后者会进入后台文档流水线，并可能通过 `chunking_func` 重新切块
- `file_path` 仅作为元数据使用，不参与 `doc_id` 计算和去重逻辑
...
当上游系统已经生成结构化实体和关系，并希望直接导入 LightRAG 时，使用这个接口。

契约如下：

- 请求体包含 `custom_kg`，以及可选的 `full_doc_id`、`directed_relation_dedup`
- `custom_kg.relationships` 使用 `src_id` 和 `tgt_id` 表示关系端点
- 实体或关系上的未知字段会保留到 `custom_properties`
- `directed_relation_dedup` 默认是 `false`，保持与上游兼容的无向端点对去重
- 当 `directed_relation_dedup` 为 `true` 时，custom KG 导入阶段会按有序 `(src_id, tgt_id)` 去重，并写入带方向的关系向量记录
- 图存储能力仍取决于配置的 `graph_storage`；默认 `NetworkXStorage` 使用无向图
...
当你需要按文档原始顺序查看已持久化的 chunk 内容时，使用这个接口。

契约如下：

- 根据 `doc_id` 查找文档
- 复用 `doc_status` 中保存的 `chunks_list`
- 返回按原始文档顺序排列的 chunk 内容
- 适用于文档详情查看和调试
...
当你需要为所有已导入的 custom_chunks 文档重新构建图谱时，使用这个接口。

契约如下：

- 服务端会扫描 `metadata.source == "custom_chunks"` 的 `doc_status` 记录
- 每个候选文档都会复用 `chunks_list` 引用的已存 `text_chunks`
- 重建流程**不会**再次调用标准文档切块器
- 任务以后台作业形式运行，并复用共享 pipeline busy 状态
- 如果当前已有其他 pipeline 作业运行，接口返回 `status="busy"`

这点非常重要：custom chunk 图谱重建必须复用已存 chunks，否则会在重建时丢失调用方自定义的 chunk 边界。

#### Web UI 行为

文档管理页现在直接暴露了这一能力：

- 顶部新增全局按钮，用于触发 `POST /documents/rebuild_custom_chunks_graph`
- 文档表新增 `来源` 列
- 通过 `/documents/import/custom-chunks` 导入的文档会显示为 `Custom Chunks`
- 文档详情弹窗可通过 `GET /documents/{doc_id}/chunks` 加载并展示已保存的 chunk 内容
