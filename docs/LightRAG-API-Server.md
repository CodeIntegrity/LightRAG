...
* `POST /prompts/entity-type/assist`: ask the runtime LLM to draft a complete YAML prompt profile from a natural-language requirement. The draft always includes `entity_types_guidance`, `entity_extraction_examples`, and `entity_extraction_json_examples`; generation is mode-complete, while extraction later chooses the text or JSON examples according to runtime `entity_extraction_use_json`. The endpoint reuses the workspace's configured LLM (preferring the `query` role binding) and does not write any file or change the active prompt. A deprecated request field `use_json` is accepted for old clients but ignored. The response carries `content` (cleaned draft), `validation`, `raw_output` (the unmodified LLM response, useful when validation fails) and the model name. The frontend asks for explicit user confirmation before applying the draft to the editor.
