...
本文档基于 2026-05-23 的静态代码与测试证据整理，未实际启动服务做运行时探测。
...
| Prompt 版本管理       | 已退役 | 旧 Prompt Management 版本库已退役；当前仅支持 `/prompts/entity-type` 文件编辑 API |
...
| `/documents/import/custom-chunks`          | POST   | 已支持 | 直接导入预切分 chunks；支持可选 `file_path`；同步执行抽取与合并；写入 `metadata.source=custom_chunks` |
| `/documents/{doc_id}/chunks`               | GET    | 已支持 | 按文档顺序查看已持久化的 chunks 内容，供文档详情与调试使用 |
| `/documents/rebuild_custom_chunks_graph`   | POST   | 已支持 | 后台重建所有 `metadata.source=custom_chunks` 文档的图谱，不重新切块 |
| `/documents/by-ids`                        | POST   | 已支持 | 按 doc_id 批量查询文档状态                                   |
| `/documents`                               | DELETE | 已支持 | 清空文档及相关存储                                           |
| `/documents/pipeline_status`               | GET    | 已支持 | 查询流水线执行状态                                           |
| `/documents`                               | GET    | 不存在 | 该路由从未在代码中实现；请使用 `/documents/paginated` 或 `/documents/by-ids` |
...
| `/documents/scan-progress`                 | GET    | 已支持 | 查询当前扫描任务进度                                         |
...
| `/graph/query`             | POST   | 已支持   | 结构化图查询接口，`scope.direction` 支持 `both/outbound/inbound`，`scope.min_depth` 支持按根实体最小跳数过滤 |
...
| `/graph/import/custom-kg`  | POST   | 已支持   | 直接导入结构化知识图谱 payload；关系使用 `src_id`/`tgt_id`，未知字段会归入 `custom_properties`；`directed_relation_dedup` 可控制 custom KG 导入阶段是否按方向去重 |
| `/graph/entity/detail`     | GET    | 已支持   | 查询实体详情，可选返回 vector data，`graph_data` 包含 `custom_properties` |
| `/graph/relation/detail`   | GET    | 已支持   | 查询关系详情，可选返回 vector data，`graph_data` 包含 `custom_properties` |
| `/graph/export`            | POST   | 已支持   | 导出图数据并以下载文件形式返回                    |

### 5. Prompt 文件管理

Prompt 文件管理路由统一挂载在 `/prompts` 前缀下。它只管理 `PROMPT_DIR/entity_type` 中的实体抽取 YAML profile 文件，不恢复旧 Prompt Management 数据库或版本 registry。

| 路径                                                 | 方法 | 状态   | 备注                                                  |
| ---------------------------------------------------- | ---- | ------ | ----------------------------------------------------- |
| `/prompts/entity-type`                               | GET  | 已支持 | 列出当前 workspace 可用的 workspace-owned 与 global 文件 |
| `/prompts/entity-type/{file_name}`                   | GET  | 已支持 | 读取并校验当前 workspace 可用文件                     |
| `/prompts/entity-type/validate`                      | POST | 已支持 | 校验 YAML 内容，不落盘                                |
| `/prompts/entity-type/{prompt_slug}/versions/{version}` | PUT  | 已支持 | 保存 `<workspace>--<prompt_slug>--v<version>.yml`，可选启用 |
| `/prompts/entity-type/activate`                      | POST | 已支持 | 校验并启用当前 runtime 的 `entity_type_prompt_file`   |
...
| `GET /documents/{doc_id}/chunks`       | `doc_status.get_by_id` + `text_chunks.get_by_ids` |
| `POST /documents/rebuild_custom_chunks_graph` | `arebuild_all_custom_chunks_graphs` |
