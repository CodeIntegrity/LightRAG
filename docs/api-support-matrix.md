...
|---|---|---|---|
| `insert_custom_chunks` / `ainsert_custom_chunks` | 是 | 否 | 未支持对外 API |
| `insert_custom_kg` / `ainsert_custom_kg` | 是 | 否 | 未支持对外 API |
| `aget_docs_by_ids` | 是 | 否 | 未支持对外 API |
| `get_entity_info` | 是 | 否 | 未支持对外 API |
| `get_relation_info` | 是 | 否 | 未支持对外 API |
...
以下是最有价值、最贴近现有核心能力、且可较低成本补齐的候选 API。

### 优先级 P1

| 建议新增 API | 对应核心能力 | 价值 |
|---|---|---|
| `POST /graph/import/custom-kg` | `ainsert_custom_kg` | 允许外部系统直接导入结构化知识图谱，不必经过文档抽取 |
| `POST /documents/import/custom-chunks` | `ainsert_custom_chunks` | 允许上游预切分系统直接喂给 LightRAG |
| `POST /documents/by-ids` | `aget_docs_by_ids` | 支持按 doc_id 批量查状态，便于任务系统联动 |
| `GET /graph/entity/detail` | `get_entity_info` | 支持前端或外部客户端查看实体完整详情 |
| `GET /graph/relation/detail` | `get_relation_info` | 支持查看关系完整详情 |

### 优先级 P2

| 建议新增 API | 对应核心能力 | 价值 |
|---|---|---|
| `POST /graph/export` | `aexport_data` | 便于导出图谱做分析、备份或迁移 |
| `POST /query/llm` 或 `POST /query/raw` | `aquery_llm` | 直接暴露“完整 query 结果”结构，减少 `/query` 与 `/query/data` 的分裂 |
...
当前没有直接证据表明以下未暴露能力已经有面向 HTTP API 的测试：
...
1. 先补文档缺口：把 prompt 更新接口补进 `lightrag/api/README.md` 与 `lightrag/api/README-zh.md`。
2. 再补 P1 接口：优先从 `documents/by-ids` 和两个 detail 查询接口开始，风险最小。
3. 若有外部知识图谱接入需求，再补 `custom-kg` 与 `custom-chunks` 导入 API。
4. 如果要拓展客户端生态，再评估是否引入 OpenAI `/v1/*` 兼容层。
