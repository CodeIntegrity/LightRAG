...
def _build_user_token():
    ...
    return auth_handler.create_token(
        "alice", role="user", metadata={"auth_mode": "disabled"}
    )
...
def test_combined_auth_accepts_user_token_when_auth_is_disabled(monkeypatch):
    monkeypatch.setattr(sys, "argv", [sys.argv[0]])
    import lightrag.api.utils_api as utils_api
    ...
    try:
        utils_api.auth_handler.accounts = {}
        monkeypatch.setattr(
            utils_api.global_args, "enable_guest_login_entry", False, raising=False
        )
        ...
        client = TestClient(app)
        response = client.get(
            "/secure", headers={"Authorization": f"Bearer {_build_user_token()}"}
        )
